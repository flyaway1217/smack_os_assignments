 ...
 *  
 *  Operating Systems Homework 3.1: Pet Store
 * 
...
#include <stdlib.h>
#include <time.h>
//#include <assert.h>  // We want to use SMACK's assert
...
int cat_enter(void) {
  ...
  //while(dogsPlaying > 0) error = pthread_cond_wait(&cvCat, &lock); // Test bug: cat plays with birds
  if(error) {
...
void play(void) {
  int i;
  // This assert only really needs to be checked once. This code was provided by the
  // instructor as a check to see if your code was working.
  //for(i = 0; i < 10; i++) {
...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  // Rather than passing in command line arguments for the thread number, we'll just 
  // hardcode it to 1 each for verification
  numCats = 2;
  numDogs = 1;
  numBirds = 2;
